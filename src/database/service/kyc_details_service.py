...
class KycDocumentsService:
    ...
    def get_rejected_kycs_between(since, to, batch_size=10000):
        session = IamEngine.get_session()
        try:
            rejected_kycs = session.query(KycStatusLogModel).filter(KycStatusLogModel.status == "rejected", KycStatusLogModel.updated_at > since, KycStatusLogModel.updated_at <= to).limit(batch_size).all()
            return rejected_kycs
        finally:
            session.close()
