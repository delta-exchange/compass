...
class ProductDetailsService:
    ...
    @staticmethod
    def generate_product_details(from_time):
        report_name = f"PRD{DateTimeUtil.get_current_date()}01"
        batch, total_products = 1, 0
        while(True):
            logger.info(f'fetching products batch {batch}')

            products_batch = ProductService.get_batch_since(batch, from_time)
            if len(products_batch) == 0: break

            products_compass = ProductDetailsService.convert_to_compass_format(products_batch)
